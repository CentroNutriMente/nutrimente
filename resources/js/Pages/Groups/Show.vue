...
<template>
    <AppLayout :title="group.name">
        ...
        <Card class="mb-5">
            <div class="flex items-start gap-4">
                <div class="w-16 h-16 rounded-full flex items-center justify-center shrink-0"
                     :class="group.tone === 'sage' ? 'bg-sage-100' : group.tone === 'lavender' ? 'bg-lavender-100' : 'bg-peach-100'">
                    <IconCircle :icon="leafIcon" :tone="group.tone" size="lg" />
                </div>
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-3 flex-wrap">
                        <h1 class="font-serif text-2xl text-stone-800">{{ group.name }}</h1>
                        <PillBadge :tone="group.tone">{{ statusLabel[group.status] }}</PillBadge>
                    </div>
                    <p class="text-sm text-stone-500 mt-1.5 leading-relaxed">{{ group.description }}</p>
                </div>
                <button @click="tab = 'impostazioni'" class="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-2xl border border-cream-200 text-sm text-stone-600 hover:bg-cream-100 transition-colors shrink-0">
            ...
            <!-- Meta -->
            <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mt-5 pt-5 border-t border-cream-100">
                <div><div class="text-xs text-stone-400">Conduttore</div><div class="text-sm font-medium text-stone-700 mt-0.5">{{ group.leader || '—' }}</div></div>
                <div><div class="text-xs text-stone-400">Periodicità</div><div class="text-sm font-medium text-stone-700 mt-0.5 capitalize">{{ group.cadence || '—' }}</div></div>
                <div><div class="text-xs text-stone-400">Prossimo incontro</div><div class="text-sm font-medium text-stone-700 mt-0.5">{{ fmtDate(group.next_meeting_at) }}</div></div>
                <div><div class="text-xs text-stone-400">Modalità</div><div class="text-sm font-medium text-stone-700 mt-0.5">{{ group.modality === 'online' ? 'Online' : 'In presenza' }}</div></div>
                <div><div class="text-xs text-stone-400">Posti disponibili</div><div class="text-sm font-medium text-stone-700 mt-0.5">{{ Math.max(0, group.capacity - group.enrolled) }} / {{ group.capacity }}</div></div>
            </div>
        </Card>

        <!-- Tabs -->
        <div class="flex items-center gap-6 border-b border-cream-200 mb-5 px-1">
            <button v-for="t in tabs" :key="t.key" @click="tab = t.key"
                :class="['pb-3 -mb-px text-sm font-medium border-b-2 transition-colors', tab === t.key ? 'border-sage-500 text-sage-700' : 'border-transparent text-stone-400 hover:text-stone-600']">
                {{ t.label }}
            </button>
        </div>
        ...
        <div v-show="tab === 'partecipanti'" class="space-y-5">
            <Card :padded="false">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-5">
                    ...
                    <button @click="showAdd = !showAdd" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-sage-500 hover:bg-sage-600 text-white text-sm font-medium shadow-soft transition-colors whitespace-nowrap">
                ...
                <div v-if="showAdd" class="px-5 pb-5">
                    <div class="rounded-2xl bg-cream-100 border border-cream-200 p-4 space-y-3">
                        <div class="flex gap-2">
                            <button @click="mode = 'patient'" :class="['px-3 py-1.5 rounded-full text-xs border', mode === 'patient' ? 'bg-sage-500 text-white border-sage-500' : 'bg-white text-stone-500 border-cream-200']">Paziente esistente</button>
                            <button @click="mode = 'external'" :class="['px-3 py-1.5 rounded-full text-xs border', mode === 'external' ? 'bg-sage-500 text-white border-sage-500' : 'bg-white text-stone-500 border-cream-200']">Contatto esterno</button>
                        </div>
                        ...
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <button @click="submitAdd" :disabled="addForm.processing" class="px-4 py-2.5 rounded-2xl bg-sage-500 hover:bg-sage-600 text-white text-sm font-medium disabled:opacity-60">Aggiungi</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="text-left text-xs text-stone-400 border-y border-cream-100">
                        ...
                        <tbody class="divide-y divide-cream-100">
                            <tr v-for="p in filtered" :key="p.id" class="hover:bg-cream-50">
                                <td class="px-5 py-3 font-medium text-stone-700">
                                    {{ p.name }}
                                    <span v-if="p.is_patient" class="ml-1 text-[10px] text-sage-500">• paziente</span>
                                </td>
                                <td class="px-5 py-3 text-stone-500">{{ p.email || '—' }}</td>
                                <td class="px-5 py-3 text-stone-500">{{ p.phone || '—' }}</td>
                                <td class="px-5 py-3">
                                    <select :value="p.status" @change="updateStatus(p, $event.target.value)"
                                        class="text-xs rounded-full border-cream-200 py-1 pl-2 pr-7 focus:border-sage-300 focus:ring-sage-200"
                                        :class="pStatusTone[p.status] === 'sage' ? 'text-sage-700 bg-sage-50' : pStatusTone[p.status] === 'peach' ? 'text-peach-500 bg-peach-50' : 'text-lavender-600 bg-lavender-50'">
                                        <option v-for="s in participantStatuses" :key="s" :value="s">{{ pStatusLabel[s] }}</option>
                                    </select>
                                </td>
                                <td class="px-5 py-3 text-right">
                                    <button @click="removeParticipant(p)" class="text-stone-300 hover:text-red-500 transition-colors">
                            ...
                            <tr v-if="filtered.length === 0"><td colspan="5" class="px-5 py-8 text-center text-stone-400">Nessun partecipante.</td></tr>
                        </tbody>
                    </table>
                </div>

                <div class="flex items-center gap-3 p-5 border-t border-cream-100">
                    <button @click="emailAll" class="inline-flex items-center gap-2 px-4 py-2 rounded-2xl border border-cream-200 text-sm text-stone-600 hover:bg-cream-100 transition-colors">
                    ...
                    <a :href="route('groups.export', group.id)" class="inline-flex items-center gap-2 px-4 py-2 rounded-2xl border border-cream-200 text-sm text-stone-600 hover:bg-cream-100 transition-colors">
            ...
            <Card :padded="false">
                <div class="px-5 pt-5 pb-3">
                    <h3 class="font-serif text-lg text-stone-800">Richieste di iscrizione</h3>
                    <p class="text-xs text-stone-400 mt-0.5">Nuove richieste ricevute dai moduli online.</p>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        <thead>
                        ...
                        <tbody class="divide-y divide-cream-100">
                            <tr v-for="r in requests" :key="r.id" class="hover:bg-cream-50">
                                <td class="px-5 py-3 font-medium text-stone-700">{{ r.name }}</td>
                                <td class="px-5 py-3 text-stone-500">{{ r.email || '—' }}</td>
                                <td class="px-5 py-3 text-stone-500">{{ r.date }}</td>
                                <td class="px-5 py-3 text-right whitespace-nowrap">
                                    <button @click="approve(r)" class="text-xs px-3 py-1.5 rounded-full bg-sage-500 hover:bg-sage-600 text-white mr-2">Approva</button>
                                    <button @click="reject(r)" class="text-xs px-3 py-1.5 rounded-full border border-cream-200 text-stone-500 hover:bg-cream-100">Rifiuta</button>
                                </td>
                            </tr>
                            <tr v-if="requests.length === 0"><td colspan="4" class="px-5 py-8 text-center text-stone-400">Nessuna richiesta in sospeso.</td></tr>
                        </tbody>
                    </table>
                </div>
            </Card>
        ...
        <Card v-show="tab === 'incontri'" class="text-center py-16 text-stone-400">Pianificazione incontri in arrivo.</Card>
        <Card v-show="tab === 'materiali'" class="text-center py-16 text-stone-400">Materiali condivisi in arrivo.</Card>

        <!-- Tab: Impostazioni -->
        <div v-show="tab === 'impostazioni'" class="grid grid-cols-1 lg:grid-cols-3 gap-5">
            <Card class="lg:col-span-2 space-y-4">
                <h3 class="font-serif text-lg text-stone-800">Impostazioni gruppo</h3>
                <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-stone-600 mb-1.5">Stato</label>
                ...
                <div class="flex items-center justify-between pt-2">
                    ...
                    <button @click="saveSettings" :disabled="settings.processing" class="px-5 py-2.5 rounded-2xl bg-sage-500 hover:bg-sage-600 text-white text-sm font-medium shadow-soft disabled:opacity-60">Salva</button>
                </div>
            </Card>

            <!-- Volantino / QR -->
            <Card class="text-center">
                <h3 class="font-serif text-lg text-stone-800 mb-2">Volantino & QR</h3>
                <p class="text-xs text-stone-400 mb-4">Le persone possono iscriversi inquadrando il QR del volantino.</p>
                <img :src="route('groups.flyer', { group: group.id, preview: 1 })" alt="QR iscrizione"
                     class="w-40 h-40 mx-auto rounded-2xl border border-cream-200 object-contain bg-white" @error="$event.target.style.display='none'" />
                <div class="mt-4 text-xs text-stone-500 break-all bg-cream-100 rounded-xl px-3 py-2">{{ group.public_url }}</div>
                <a :href="route('groups.flyer', group.id)" target="_blank"
                   class="inline-flex items-center gap-2 mt-4 px-4 py-2.5 rounded-2xl bg-sage-500 hover:bg-sage-600 text-white text-sm font-medium shadow-soft transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v12m0 0l-4-4m4 4l4-4M4 20h16"/></svg>
                    Scarica volantino
                </a>
            </Card>
